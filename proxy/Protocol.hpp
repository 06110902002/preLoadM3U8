        ...
        if (pid == 0) { //child
            ...
            if (method == "POST") // 将正文中的参数通过管道传递给CGI程序
            {
                ...
                while (total < content_length &&
                       (size = write(output[1], start + total, request_body.size() - total)) > 0) {
                    total += size;
                }
            }
            ...
            if (ret == pid) {
                if (WIFEXITED(status)) { // 子进程正常退出
                    if (WEXITSTATUS(status) == 0) { // 子进程退出码结果正确
                        LOG("CGI program exits normally with correct results");
                        code = OK;
                    } else {
                        LOG("CGI program exits normally with incorrect results");
                        code = BAD_REQUEST;
                    }
                } else {
                    LOG("CGI program exits abnormally");
                    code = INTERNAL_SERVER_ERROR;
                }
            }
...
public:
    ...
    void HandlerHttpRequest() {
        ...
        if (_http_request._method == "GET") {
            ...
            printf("466-----收到请求 url = %s\n", _http_request._uri.c_str());
        ...
        {
            LOG(_http_request._path + "NOT_FOUND");
            code = NOT_FOUND;
            return;
        }
...
public:
    ...
    void HandlerRequest(int sock) {
        ...
        if (!ep->IsStop()) {
            ...
            if (ep->IsStop()) {
                LOG("SendHttpResponse  Stop Send HttpResponse");
            }
        } else {
            printf("RecvHttpRequest  Stop handler Response\n");
        }
        close(sock); //响应完毕，关闭与该客户端建立的套接字
        delete ep;
        LOG("handler request end");
    }
};
